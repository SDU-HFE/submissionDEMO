...
//                      从本地数据库填入数据
        final DateBaseHelper dateBaseHelper = new DateBaseHelper(this, "AccountData", null, 1);
...
//        如果有数据，直接执行本地登录
        if (Account!=null&&password!=null&&Account!=""&&password!="") {
            HttpUtil.sendHttpRequest("http://120.24.33.180:8081/login?user=" + accountEdit.getText().toString()+ "&&password=" + passwordEdit.getText().toString());
        ...
        mlogin.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (accountEdit.getText() == null || accountEdit.getText().length() == 0) {
                    ...
                    HttpUtil.sendHttpRequest("http://120.24.33.180:8081/login?user=" + AccountNow + "&&password=" + passwordNow);
